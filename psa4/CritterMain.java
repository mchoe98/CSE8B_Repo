...
class CritterModel extends Observable implements Iterable<Critter> {
	...
	private Critter fight(Critter critter1, Critter critter2) {
		...
		if (((classState1.isAsleep() || state1.isAsleep()) && (classState2.isAsleep() || state2.isAsleep())) ||
			...
			// TODO new code
			double boundary = 1.00; //Should change to 0.96 or so.

			if (weapon1 == weapon2) {
				// tie
				winner = (Math.random() < 0.5) ? critter1 : critter2;

				// Hans - Editing the following section to introduce probability:
					...
					|| (weapon1 == Critter.Attack.ROAR
						&& weapon2 == Critter.Attack.SCRATCH
						&& (Math.random() < boundary) )

					|| (weapon1 == Critter.Attack.SCRATCH
						&& weapon2 == Critter.Attack.POUNCE
						&& (Math.random() < boundary))

					|| (weapon1 == Critter.Attack.POUNCE
						&& weapon2 == Critter.Attack.ROAR
						&& (Math.random() < boundary))
				  ) // End if statement

			{
				// The new condition is this rock paper and scissors method is the same
				// system as before, but it now has only a 97% chance of it happening.
		...
		winner.win();
		loser.lose();
		winner.playSound("./sounds/slaughter.wav");
